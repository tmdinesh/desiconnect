...
export const getAdminStats = async (req: Request, res: Response) => {
  try {
    ...
    const totalRevenue = fulfilledOrders.reduce((sum, order) => sum + parseFloat(String(order.totalPrice)), 0);
...
export const createSeller = async (req: Request, res: Response) => {
  try {
    ...
    if (existingSeller) return res.status(409).json({ message: 'Email already in use' });
...
export const getPendingProducts = async (req: Request, res: Response) => {
  try {
    const pendingProducts = await storage.getPendingProducts();
    const productsWithSellerInfo = await Promise.all(pendingProducts.map(async (product) => {
...
export const approveProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (product.status !== 'pending') return res.status(400).json({ message: 'Product is not pending approval' });
...
export const deleteProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (productInOrders) return res.status(400).json({ message: 'Cannot delete product referenced in orders' });
